          ...
          <div key={index} className='card flex flex-col pb-5 rounded-md' style={{ boxShadow: "rgba(50, 50, 93, 0.25) 0px 50px 100px -20px, rgba(0, 0, 0, 0.3) 0px 30px 60px -30px" }}>
            ...
            <div className='px-3'>
              ...
              <div className='flex justify-between items-center pt-3'>
                <p className='price'>
                  <span className='text-2xl text-[#6477f2] font-bold mr-2 text-shadow-amber-950'>${item?.price}</span>
                  <span className='text-sm line-through'>219.99</span>
                </p>
              ...
              <div className='rating py-3'>
                <div className='flex'>
                  <p className='flex space-x-1 items-center'>
                    {Array(item?.rating | 4).fill().map((_, index) => (
                      <Star key={index} fill='orange' stroke='orange' size={15} />
                    ))}
                    <span className='text-sm ml-3'>{item?.rating || 4}</span>
                  </p>
                </div>
              </div>

              <button className='bg-orange-600 py-2 px-2 rounded-sm text-sm text-white w-full flex justify-center cursor-pointer'>
                <span className='mr-3' onClick={() => handlerAddToCart()}>
                  <ShoppingCart />
                </span> Add to Cart
              </button>
            </div>
